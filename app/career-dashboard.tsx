...
function JobCard({ job, saved, onSave, onOpen, onLetter }: { job: Job; saved: boolean; onSave: () => void; onOpen: () => void; onLetter: () => void }) {
  return <article className="job-card">
    ...
    <div className="match-row"><div className="score-ring" style={{ "--score": `${job.matchScore * 3.6}deg` } as React.CSSProperties}><span>{job.matchScore}%</span></div><div><strong>Excellent match</strong><p>{job.matchingSkills.length} of {job.requiredSkills.length} core skills align</p></div></div>
...
export function CareerDashboard({ initialData }: { initialData: { student: Student; jobs: Job[] } }) {
  ...
  return <div className="app-shell">
    ...
    {selected ? <div className="detail-drawer" role="dialog" aria-modal="true" aria-label="Job match details"><button className="drawer-close" onClick={() => setSelected(null)}>×</button><div className="company-logo large" style={{ background: selected.companyColour }}>{selected.companyInitials}</div><span className="eyebrow">{selected.matchScore}% profile match</span><h2>{selected.position}</h2><p className="drawer-company">{selected.company} · {selected.location} · {selected.workMode}</p><div className="drawer-summary">{selected.description}</div><h3>Why you match</h3><ul className="fit-list">{selected.matchingSkills.map((skill) => <li key={skill}><span>✓</span><div><strong>{skill} is a direct match</strong><p>Your profile demonstrates this core requirement.</p></div></li>)}<li><span>✓</span><div><strong>Location and salary align</strong><p>{selected.location} is in your preferences and the range overlaps your target.</p></div></li>{selected.missingSkills.map((skill) => <li className="gap" key={skill}><span>↗</span><div><strong>{skill} is an opportunity to grow</strong><p>Consider highlighting adjacent project experience.</p></div></li>)}</ul><div className="job-facts"><span><small>Experience</small>{selected.yearsExperience === 0 ? "Graduate friendly" : `${selected.yearsExperience}+ years`}</span><span><small>Salary</small>{money(selected.salaryMin)}–{money(selected.salaryMax)}</span><span><small>Visa</small>{selected.visaRequirement}</span></div><div className="drawer-actions"><button className="secondary-button" onClick={() => toggleSave(selected.id)}>{saved.includes(selected.id) ? "Saved ♥" : "Save job"}</button><button className="primary-button" onClick={() => setLetterJob(selected)}>Create cover letter</button></div></div> : null}
